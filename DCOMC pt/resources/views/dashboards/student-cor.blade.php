...
<head>
    ...
    <title>Certificate of Registration - DCOMC</title>
    @include('layouts.partials.offline-assets')
    @include('dashboards.partials.dcc-certification-styles')
</head>
<body class="bg-gray-100">
    <div class="dcc-print-page max-w-[210mm] mx-auto p-6" style="background: #fff;">
        <div class="no-print flex flex-wrap items-center justify-between gap-4 mb-4">
            <h1 class="text-2xl font-bold text-gray-800">Certificate of Registration (COR)</h1>
            <div class="flex flex-wrap items-center gap-3">
                <form method="GET" action="{{ $cor_form_action ?? route('student.cor') }}" class="flex items-center gap-2" id="corSchoolYearForm">
                    <label for="school_year" class="text-sm font-medium text-gray-700">School year:</label>
                    <select name="school_year" id="school_year" onchange="this.form.submit()" class="border border-gray-300 rounded px-3 py-2 text-sm bg-white min-w-[140px]">
                        <option value="">Current</option>
                        @foreach($availableSchoolYears ?? [] as $yr)
                            <option value="{{ $yr }}" {{ (($selectedSchoolYear ?? '') === $yr) ? 'selected' : '' }}>{{ $yr }}</option>
                        @endforeach
                    </select>
                </form>
                <a href="{{ $back_url ?? route('student.dashboard') }}" class="px-4 py-2 bg-gray-700 text-white rounded no-underline">Back</a>
                <button type="button" onclick="window.print()" class="px-4 py-2 bg-green-700 text-white rounded">Print</button>
            </div>
        </div>
        ...
        @if(!empty($corFromRegistrar))
        <div class="no-print mb-4 p-3 rounded border-2 border-amber-400 bg-amber-50 text-amber-900 text-sm">
            <strong>Irregular COR.</strong> This Certificate of Registration is issued by the <strong>Registrar</strong> (Create Schedule), not from the Dean's Schedule by Program.
        </div>
        @endif

        @include('dashboards.partials.cor-institutional-header')

        {{-- STUDENT GENERAL INFORMATION (boxed, two-column) --}}
        ...
        {{-- Subjects Table: Code, Subject Title, Units, Schedule, Faculty Signature --}}
        ...
        <div style="display: flex; flex-wrap: wrap; justify-content: space-between; gap: 24px; align-items: flex-start;">
            ...
            {{-- Student signature + Approved by (right) --}}
            <div style="flex: 1; min-width: 280px;">
                @include('dashboards.partials.cor-signatory')
            </div>
        </div>

        @include('dashboards.partials.cor-footer')
    </div>
</body>
</html>
