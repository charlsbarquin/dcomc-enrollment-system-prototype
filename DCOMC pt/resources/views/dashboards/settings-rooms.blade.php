...
<head>
    ...
    <title>Settings - Rooms - DCOMC</title>
    ...
    <main class="flex-1 flex flex-col min-w-0 overflow-hidden">
        <div class="flex-1 overflow-y-auto p-6 md:p-8 forms-canvas">
            <div class="w-full max-w-4xl mx-auto">
                ...
                <section class="w-full hero-gradient rounded-2xl shadow-lg p-6 sm:p-8 text-white mb-6">
                    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                        <div>
                            ...
                            <p class="text-white/90 text-sm sm:text-base font-data">Add rooms for scheduling. Department scope controls which dean can assign them (education, entrepreneurship, or all).</p>
                        </div>
                        @if(request()->routeIs('admin.settings.*'))
                ...
                {{-- Add Room card (Explorer style) — 10px solid blue top border --}}
                <div class="bg-white shadow-2xl rounded-xl border border-gray-200 border-t-[10px] border-t-[#1E40AF] px-6 py-5 mb-6">
                    ...
                    <form method="POST" action="{{ request()->routeIs('admin.settings.*') ? route('admin.settings.rooms.store') : ($isDean ?? false ? route('dean.settings.rooms.store') : route('registrar.settings.rooms.store')) }}" class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        @csrf
                        <div>
                ...
                <div class="rounded-xl overflow-hidden border border-gray-200 shadow-2xl bg-white">
                    ...
                    <div class="overflow-x-auto">
                        @if($rooms->isEmpty())
                            <p class="text-sm text-gray-500 font-data p-6">No rooms yet. Add one above.</p>
                        @else
                            <table class="w-full text-sm font-data admin-table-wrap">
                                <thead class="table-header-dcomc">
                                    <tr>
                                        ...
                                        <th class="py-3 px-4 text-left text-white">Code</th>
                                        <th class="py-3 px-4 text-left text-white">Capacity</th>
