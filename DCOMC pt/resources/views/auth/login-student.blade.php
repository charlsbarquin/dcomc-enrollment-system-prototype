...
@section('title', 'Sign in — Student Portal')

@section('portal_name', 'Student Portal Login')
...
@section('content')
    ...
    <form method="POST" action="/login" class="js-auth-form" aria-label="Student portal login" data-loading-text="Signing in…">
        @csrf
        <input type="hidden" name="portal_type" value="student">

        <div class="mb-4">
            <label for="email">Student ID / School ID</label>
            <input type="text" name="email" id="email" value="{{ old('email') }}" required autofocus placeholder="Enter your Student ID or School ID" autocomplete="username" aria-required="true">
        </div>
        <div class="mb-4">
        ...
        <div class="login-remember-forgot">
            ...
            <a href="{{ route('password.request') }}?portal=student" aria-label="Forgot your password?">Forgot your password?</a>
        </div>
        <button type="submit" class="btn-login js-submit-btn">Sign in</button>
    </form>
@endsection

@section('badge', 'Student Access Only')
    ...
    <script>
        if (!/Mobi|Android|iPhone|iPad|iPod/i.test(navigator.userAgent)) {
            ...
            document.addEventListener('keydown', function (e) {
                ...
                if (e.key === 'ArrowRight') { e.preventDefault(); showSwitchPopup('/dcomc-login'); }
                else if (e.key === 'ArrowLeft') { e.preventDefault(); showSwitchPopup('/admin-login'); }
            });
